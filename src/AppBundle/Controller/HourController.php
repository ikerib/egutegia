...
class HourController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        return $this->render('hour/index.html.twig', array(
            'hours' => $hours,
        ));
    }
    ...
    public function newAction(Request $request, $calendarid)
    {
        ...
        $form = $this->createForm('AppBundle\Form\HourType', $hour, array(
            'action' => $this->generateUrl('admin_hour_new', array('calendarid'=> $calendarid)),
            'method' => 'POST'
        ));
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirect(
                $this->generateUrl( 'admin_calendar_edit', array( 'id' => $calendar->getId())). '#konpentsatuak'
            );
        }

        return $this->render('hour/new.html.twig', array(
            'hour' => $hour,
    ...
    public function editAction(Request $request, Hour $hour)
    {
        ...
        $editForm = $this->createForm('AppBundle\Form\HourType', $hour, array(
            'action' => $this->generateUrl('admin_hour_edit', array('id'=> $hour->getId())),
            'method' => 'POST'
        ));
        ...
        if ($editForm->isSubmitted() && $editForm->isValid()) {
            ...
            $temp = floatval($OriginalEntityData["total"]);

            $calendar->setHoursCompensed( floatval($calendar->getHoursCompensed()) - $temp + floatval($data->getTotal()) );
        ...
        return $this->render('hour/edit.html.twig', array(
            'hour' => $hour,
    ...
    public function deleteAction(Request $request, Hour $hour)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $calendar->setHoursCompensed( floatval( $calendar->getHoursCompensed() - floatval( $hour->getTotal() ) ) );
    ...
    private function createDeleteForm(Hour $hour)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('admin_hour_delete', array('id' => $hour->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
